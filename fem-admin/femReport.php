...
?>
<section class="mt-10">
<table id="femReportTable" class="display" style="width:100%">
<thead>
            <tr>
                <th>ID Number</th>
                <th>Employee</th>
        ...
        <tbody>
              <?php
                $a=1;
                  ...
                  $sql="SELECT user.username, user.name, COUNT(request.rating_final) as numberOfJO, SUM(request.rating_final) as TotalofRates, ROUND((SUM(request.rating_final))/(COUNT(request.rating_final)), 1) as totalRating FROM user INNER JOIN request ON user.username = request.assignedPersonnel WHERE user.level = 'fem'  AND request.status2 = 'rated' GROUP BY user.username, request.assignedPersonnelName;";
